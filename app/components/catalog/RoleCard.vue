    ...
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        ...
        <p
          ...
          class="mt-1.5 line-clamp-2 break-words text-sm leading-6 text-ink-soft"
        >
          {{ role.description }}
        </p>
      </div>
      <span
        v-if="selected"
        class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-accent text-white"
      >
        <svg
          width="12"
          height="12"
      ...
      <span
        v-else
        class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-border-subtle text-ink-soft opacity-0 transition-opacity group-hover:opacity-100"
      >
        <svg
          width="10"
          height="10"
