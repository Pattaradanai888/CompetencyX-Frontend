...
if (sessionSnapshot.current_question || sessionSnapshot.status !== 'completed') {
...
const rankedRoles = computed(() => sortRankedRolesDescending(results.value?.ranked_roles ?? []))
...
<template>
  <main v-if="results" id="main-content" class="page-wrap">
    ...
    <div class="assessment-stagebar mt-6 flex flex-wrap items-center justify-between gap-3 rounded-[1.5rem] border border-black/6 bg-white/60 px-4 py-3">
      ...
      <p class="text-sm text-[var(--cx-ink-soft)]">Your recommendation is based on the latest completed session.</p>
    </div>
    ...
    >
      <section class="glass-panel rounded-[2.2rem] p-6 md:p-8">
        ...
        <h1 class="mt-4 font-display text-4xl leading-tight text-[var(--cx-ink)] md:text-6xl">
          {{ results.best_fit_role?.name || results.preferred_role?.name || 'Assessment complete' }}
        </h1>
        ...
        <div class="mt-6 rounded-[1.6rem] border border-black/6 bg-white/75 p-5">
          ...
          <p class="mt-2 text-sm font-semibold text-[var(--cx-ink)]">
            Resolution: {{ getRoleResolutionLabel(results.role_resolution_status) }}
          </p>
          <p class="mt-2 text-sm leading-7 text-[var(--cx-ink-soft)]">
            Role answers: {{ results.milestones.answered_role_questions }}. Skill answers:
            {{ results.milestones.answered_skill_questions }}.
          </p>
        </div>
      </section>
        ...
        <div class="mt-6 space-y-3">
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <div>
                <p class="text-sm font-semibold text-[var(--cx-ink)]">{{ role.name }}</p>
                <p class="mt-2 text-sm leading-6 text-[var(--cx-ink-soft)]">
                  {{ role.top_supporting_pillars.join(', ') || 'No supporting pillar details available.' }}
                </p>
              </div>
      ...
      <section class="paper-panel rounded-[2rem] p-6">
        ...
        <div class="mt-6 space-y-3">
          <div
            ...
            class="rounded-[1.35rem] border border-black/6 bg-white/75 p-4"
          >
            <p class="text-sm font-semibold text-[var(--cx-ink)]">{{ topic.title }}</p>
            <p class="mt-2 text-sm leading-6 text-[var(--cx-ink-soft)]">
              {{ topic.description || 'This topic is highlighted as a likely gap for the preferred role path.' }}
            </p>
          </div>
          <p
    ...
    <section class="mt-8 paper-panel rounded-[2rem] p-6">
      ...
      <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <div
          ...
          class="rounded-[1.35rem] border border-black/6 bg-white/75 p-4"
        >
          <p class="text-sm font-semibold text-[var(--cx-ink)]">{{ pillar.label }}</p>
          <p class="mt-3 text-2xl font-semibold text-[var(--cx-ink)]">
            {{ formatConfidencePercent(pillar.normalized_score) }}
          </p>
          <p class="mt-2 text-sm leading-6 text-[var(--cx-ink-soft)]">
            Raw score {{ pillar.raw_score }} from {{ pillar.evidence_count }} evidence point{{
