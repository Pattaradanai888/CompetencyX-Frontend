...
const qStart = lines.findIndex(
  (l) => l.includes('<section') && l.includes('v-if="!isRoadmapsComplete"'),
)
const qEnd = lines.findIndex(
  (l) =>
    l.includes('</section>') &&
    l > qStart &&
    lines[l - 1].includes('</section>'),
)
const dashStart = lines.findIndex((l) =>
  l.includes('<template v-if="isRoadmapsComplete">'),
)
const dashEnd = lines.findIndex(
  (l) => l.includes('</template>') && l > dashStart && l > qEnd,
)
