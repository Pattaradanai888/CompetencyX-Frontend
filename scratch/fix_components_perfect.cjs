...
const hasRoleAnswersStart = originalScript.indexOf('const hasRoleAnswers = computed(() => {');
...
if (fs.existsSync(qFile)) {
  ...
  const newContent = content.substring(0, qScriptStart) + questionnaireScript + content.substring(qScriptEnd);
...
if (fs.existsSync(dFile)) {
  ...
  const newContent = content.substring(0, dScriptStart) + dashboardScript + content.substring(dScriptEnd);
